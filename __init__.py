...
from .src.patch import apply_dype_to_flux, apply_dype_to_chroma, apply_dype_to_zimage, apply_dype_to_wan
...
class DyPE_ZImage(io.ComfyNode):
    """
    Applies DyPE (Dynamic Position Extrapolation) to a FLUX model.
    This allows generating images at resolutions far beyond the model's training scale
    by dynamically adjusting positional encodings and the noise schedule.
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="DyPE_ZImage",
            display_name="DyPE for Z-Image",
            category="model_patches/unet",
            description="Applies DyPE (Dynamic Position Extrapolation) to a Z-Image model for ultra-high-resolution generation.",
            inputs=[
                io.Model.Input(
                    "model",
                    tooltip="The Z-Image model to patch with DyPE.",
                ),
                io.Int.Input(
                    "width",
                    default=1024, min=16, max=8192, step=8,
            ...
            outputs=[
                io.Model.Output(
                    display_name="Patched Model",
                    tooltip="The Z-Image model patched with DyPE.",
                ),
            ],
        )
    ...
    def execute(cls, model, width: int, height: int, method: str, enable_dype: bool, dype_exponent: float = 2.0, shift: float = 1.0, start_at_sigma:float = 1.0, base_resolution:int = 1024) -> io.NodeOutput:
        ...
        if not hasattr(model.model, "diffusion_model") or not hasattr(model.model.diffusion_model, "rope_embedder"):
             raise ValueError("This node is only compatible with Z-Image models.")

        if base_resolution == 0:
            base_resolution = math.isqrt(width * height)

        patched_model = apply_dype_to_zimage(model, width, height, method, enable_dype, dype_exponent, shift, start_at_sigma, base_resolution)
        return io.NodeOutput(patched_model)
...
class DyPEExtension(ComfyExtension):
    ...
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [DyPE_FLUX, DyPE_Chroma, DyPE_ZImage, DyPE_Wan]
